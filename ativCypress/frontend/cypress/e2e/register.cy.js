describe('Cadastro de Tarefas', () => {
  it('Deve adicionar uma nova tarefa', () => {
    ...
    cy.get('input[placeholder="Descrição"]').type('Criar testes do NERA');
    ...
    cy.contains('Criar testes do NERA - Concluída').should('be.visible');
  });
  ...
  it('Deve aplicar estilos corretamente ao adicionar uma tarefa pendente', () => {
    ...
    cy.get('input[placeholder="Descrição"]').type('Documentar Cypress Test');
    cy.get('button').contains('Adicionar Tarefa').click();
    ...
    cy.contains('Documentar Cypress Test - Pendente')
      .should('have.class', 'pending')
      .and('have.css', 'color', 'rgb(200, 192, 86)');
  });
});
...
describe('Editar Tarefas', () => {
  ...
  it('Deve editar uma tarefa existente', () => {
    ...
    cy.get('#editardescriacao').clear().type('Tarefa Editada');
    cy.get('input[type="checkbox"]').check();
    ...
    cy.contains('Tarefa Editada - Concluída').should('be.visible');
  });
});
